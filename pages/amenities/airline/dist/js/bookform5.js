document.addEventListener("DOMContentLoaded", async function () {
    ...
    // Fetch airline companies and locations from the backend
    try {
        const airlineResponse = await fetch("http://localhost:3000/getAllairLocations", {
            method: "GET",
            headers: {
        ...
        if (airlineResponse.status === 200) {
            ...
            // Extract airline companies and locations from the response
            const airlineCompanies = [...new Set(airlineData.locations.map(location => location.airlineCompany))];
            const sourceCountries = [...new Set(airlineData.locations.map(location => location.country))];
            const destinationCountries = [...new Set(airlineData.locations.map(location => location.country))];

            // Create dropdown options for airline companies
            const airlineCompanyDropdown = document.getElementById('airlineCompany');
            createDropdownOptions(airlineCompanies, airlineCompanyDropdown);

            // Create dropdown options for source countries
            const sourceCountryDropdown = document.getElementById('flightSource');
            createDropdownOptions(sourceCountries, sourceCountryDropdown);

            // Create dropdown options for destination countries
            const destinationCountryDropdown = document.getElementById('flightDestination');
            createDropdownOptions(destinationCountries, destinationCountryDropdown);
        } else {
            console.error("Failed to retrieve airline companies and locations");
    ...
    window.submitAirlineInfo = async function (event) {
        ...
        const airlineCompanyInput = airlineInfoForm.querySelector('select[name="airlineCompany"]');
    ...
    window.submitFlightInfo = async function (event) {
        ...
        const flightSourceInput = flightInfoForm.querySelector('select[name="flightSource"]');
        const flightDestinationInput = flightInfoForm.querySelector('select[name="flightDestination"]');
        const numberOfPeopleInput = flightInfoForm.querySelector('select[name="numberOfPeople"]');
        const flightCostInput = flightInfoForm.querySelector('input[name="flightCost"]');
        ...
        if (flightTimeInput && flightDateInput && flightSourceInput && flightDestinationInput && numberOfPeopleInput && flightCostInput) {
            ...
            try {
                ...
                if (response.status === 201) {
                    ...
                    window.location.href = "../../../bill/airbill.html";
                } else {
